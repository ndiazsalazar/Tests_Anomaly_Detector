...
def plot_signal(T_original, dataset, trainPredictPlot, testPredictPlot, original, scaler, key, title='', line_width=1, alpha=1, color='k', subplots=False, show_grid=True, fig_size=(12, 5), transient = None, t_transient = None):
    ...
    plt.plot(T_original[1:], scaler.inverse_transform(dataset), '.-b', label='Diff Original', linewidth=0.4, markersize=0.8)
...
###################################
# Folder for plots
folder = 'FFNN_LSTM_velocity_rms_white_noise2' + '/'
...
for key in SynObsSignalsDiff.keys():
    ...
    for Nneurons in N_neurons:
        ...
        model = FeedForward(Nneurons, batch_size, epochs, trainX, trainY, input_dim, outputnumber = 5)
        ...
        fig = plot_signal(T_original, dataset, trainPredictPlot, testPredictPlot, original, scaler, key, title = title, line_width = 1, alpha = 1, color = 'k', subplots = False, show_grid = True, fig_size = (12, 5), transient = transient, t_transient = T_transient)
