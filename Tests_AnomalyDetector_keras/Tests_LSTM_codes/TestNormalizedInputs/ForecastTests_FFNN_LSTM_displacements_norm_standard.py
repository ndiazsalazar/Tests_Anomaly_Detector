...
def create_dataset3(dataset, look_back=1, steps_ahead = 5, folderpickle = 'scalefiles', normalization = 'standard'):
    dataX, dataY = [], []
    for i in range(len(dataset)-look_back-steps_ahead+1):

        dataset_sample = dataset[i:(i+look_back) + steps_ahead]
        ...
        scaler = scaler.fit(dataset_sample)
        dataset_sample = scaler.transform(dataset_sample)

        dataX.append(dataset_sample[0:(0+look_back), 0])
        dataY.append(dataset_sample[0 + look_back: 0 + look_back + steps_ahead, 0])

        # save the scaler
        pickle.dump(scaler, open(folderpickle + '/scaler_' + str(i) + '.pkl', 'wb'))
...
def re_scale(trainPredict, scaleFolder = 'Scale'):

    n_trainsamples, ahead = trainPredict.shape

    trainPredict_sc = np.zeros_like(trainPredict)

    for i in range(n_trainsamples):
        filename = scaleFolder + '/scaler_' + str(i) + '.pkl'
        infile = open(filename, 'rb')
        scaler = pickle.load(infile)
        #print('*o*', trainPredict.shape)

        sample = trainPredict[i, :]
        new = sample.reshape(sample.size, 1)

        inv = scaler.inverse_transform(new)
        #print('invshape', inv.shape)
        trainPredict_sc[i,:] = inv.flatten()

        infile.close()

    print('trainpredictsc.shape', trainPredict_sc.shape)

    return trainPredict_sc





def plot_signal2(T_original, dataset, trainPredictPlot, testPredictPlot, original, key, title='', line_width=1, alpha=1, color='k', subplots=False, show_grid=True, fig_size=(12, 5), transient = None, t_transient = None, velocity=True):
    ...
    if velocity:
        ...
        plt.plot(T_original[1:], dataset, '.-b', label='Original data', linewidth=0.4, markersize=0.8)
        plt.plot(T_original[1:], trainPredictPlot, '.r', label='Forecast on the training set', markersize=0.8)
        plt.plot(T_original[1:], testPredictPlot, '.g', label='Forecast on the testing set', markersize=0.8)
        #plt.legend(loc = 'lower left')

    else:

        plt.plot(T_original, dataset, '.-b', label='Original data', linewidth=0.4, markersize=0.8)
        plt.plot(T_original, trainPredictPlot, '.r', label='Forecast on the training set', markersize=0.8)
        plt.plot(T_original, testPredictPlot, '.g', label='Forecast on the testing set', markersize=0.8)
        #plt.legend(loc = 'lower left')
    ...
    errorTrain = dataset - trainPredictPlot
    errorTest = dataset - testPredictPlot
...
###################################
# Folder for plots
folder = 'Norm_FFNN_LSTM_displacement_rms_white_noise2_5steps_ahead_Ndays365' + '/'
...
# lot signals
folderS = folder + 'Synthetic_36Signals_moredata' + '/'
...
Ndays = 365
N_neurons = [5]
#caso = ['2_secular','11_seasonal_secular_transient1'] #'29_transient2'] ,'30_transient2_noise' ]
...
for key in SynObsSignalsDiff.keys():
    ...
    for Nneurons in N_neurons:
        ...
        scaleFolderTrain = 'scalesFolderTrain_' + key
        scaleFolderTest = 'scalesFolderTest_' + key

        if not os.path.exists(scaleFolderTrain):
            os.makedirs(scaleFolderTrain)

        if not os.path.exists(scaleFolderTest):
            os.makedirs(scaleFolderTest)



        trainX, trainY = create_dataset3(train, look_back, steps_ahead = stepsahead, folderpickle = scaleFolderTrain)
        testX, testY = create_dataset3(test, look_back, steps_ahead = stepsahead, folderpickle = scaleFolderTest)

        # Normalize the samples used as input.
        ...
        trainPredict_sc = re_scale(trainPredict, scaleFolder = scaleFolderTrain)
        testPredict_sc = re_scale(testPredict, scaleFolder = scaleFolderTest)
        ...
        for i in range(n_aheads):
             trainPredictPlot[i * ahead + look_back: i * ahead + ahead + look_back, 0] = trainPredict_sc[i * ahead, :]
        ...
        for i in range(n_aheads_test):
            testPredictPlot[train_size + i * ahead + look_back: train_size + i * ahead + ahead + look_back, 0] = testPredict_sc[i * ahead, :]
        ...
        fig = plot_signal2(T_original, dataset, trainPredictPlot, testPredictPlot, original, key, title = title, line_width = 1, alpha = 1, color = 'k', subplots = False, show_grid = True, fig_size = (12, 5), transient = transient, t_transient = T_transient, velocity=False)
...
for key in caso:
    ...
    for Nunits in N_neurons:
        ...
        trainX, trainY = create_dataset3(train, look_back, steps_ahead = stepsahead, folderpickle = scaleFolderTrain, normalization = 'standard')
        testX, testY = create_dataset3(test, look_back, steps_ahead= stepsahead, folderpickle  = scaleFolderTest, normalization = 'standard')
        ...
        trainPredict = model.predict(trainX, batch_size=batch_size)
        testPredict = model.predict(testX, batch_size=batch_size)
        print('trainPredict_LSTM', testPredict.shape)
